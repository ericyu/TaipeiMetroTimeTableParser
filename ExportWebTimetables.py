...
def getPageHeaderWithSwitchTable(
    code,
    codeText,
    effectiveFrom,
    allDaysPatterns,
    ...
    result += getSwitchPatternDirectionTable(
        code, allDaysPatterns, allDirections, currentDaysPattern, currentDirection
    )
    result += "</div></div>"
    return result


def getSwitchPatternDirectionTable(
    code, allDaysPatterns, allDirections, currentDaysPattern, currentDirection
):
...
def getLineTimetable(stations, data, direction, daysPattern):
    ...
    for i, station in enumerate(stations):
        if i != len(stations) - 1:
            ...
            appendLink = False
        url = "../stations/{}-{}-{}.html".format(
            station, DirectionToCode[direction], daysPattern
        )
        result += "<tr><td>"
        if appendLink:
...
def printLineFile(
    ...
    f.write(
        getPageHeaderWithSwitchTable(
            lineCode,
            lineCodeToName[lineCode],
            effectiveFrom,
            allDaysPatterns,
            allDirections,
            currentDaysPattern,
...
def getStationTimetable(data):
    ...
    for record in data:
        ...
        byHourData[hour].append(
            {"Min": minute, "Dst": record["Dst"], "First": record["First"]}
        )
    ...
    for hour in sorted(byHourData.keys()):
        ...
        for minRec in minData:
            ...
            if minRec["First"]:
                ...
                firstMarkUsed = True
            result += '</div><div class="minute">{:02d}</div></div>'.format(
                minRec["Min"]
            )
        result += "</td></tr>"
    result += "</tbody></table></div>"
...
def printStationFile(
    ...
    fileName = "{}/stations/{}-{}-{}.html".format(
        outputDir, stationCode, DirectionToCode[currentDirection], currentDaysPattern
    )
    ...
    f.write(
        getHtmlFileHeader(
            "{} {}時刻表".format(stationCode, stationMapping.CodeToName(stationCode))
        )
    )
    f.write(
        getPageHeaderWithSwitchTable(
            stationCode,
            stationMapping.CodeToName(stationCode),
            effectiveFrom,
            allDaysPatterns,
            allDirections,
            currentDaysPattern,
...
def printStationSummaryPage(stationName, data):
    ...
    f.write(
        '<div class="header-wrapper"><div id="main-header">{} 時刻表</div></div>'.format(
            stationName
        )
    )
    ...
    for (lineCode, stationCode), value in byLine.items():
        f.write(
            '<div class="line-header"><span class="route-box summary-line-name {}">{}</span> <span class="route-box route-box-larger {}">{}</span></div>'.format(
                lineCode.lower(),
                lineCodeToName[lineCode],
                lineCode.lower(),
                stationCode,
        ...
        for pattern in allDaysPatterns:
            f.write("<td>")
            directions = sorted(
                list(set([x[0] for x in value])), key=lambda k: DirectionToCode[k]
            )
            for direction in directions:
                f.write(
                    '<p><a href="../stations/{}-{}-{}.html">{}方向</a></p>'.format(
...
def processLineTimeTable(inputFile):
        ...
        for timetable in direction["Timetables"]:
            ...
            for train in timetable["Trains"]:
                tableData.append(
                    list(
                        map(
                            lambda k: (k["StationCode"], k["DepTime"]),
                            train["Schedule"],
                        )
                    )
                )
    ...
    for key in stationData.keys():
        ...
        summaryByStation[stationName].append(
            {
                "LineCode": lineCode,
                "StationCode": key[0],
                "Direction": key[1],
